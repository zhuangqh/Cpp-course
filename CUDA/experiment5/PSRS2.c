...
void Psrs_Main() {
  ...
  if (SumID>1) {
    ...
    if (MyID==0) {
      ...
      j = 0;
      for (i=1;i<SumID;i++)
        MPI_Irecv(&temp1[i*Spt], sizeof(int)*Spt,
      ...
      /*处理器P0将这P-1个主元播送到
        所有处理器中*/
      MPI_Bcast(temp1, sizeof(int)*(1+Spt), MPI_CHAR, 0,
                MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
    }
    else {
      MPI_Send(temp1,sizeof(int)*Spt, MPI_CHAR,0,
               ALLTOONE_TYPE+MyID, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      MPI_Bcast(temp1, sizeof(int)*(1+Spt),
      ...
      if (i==MyID) {
        ...
        for (n=0;n<SumID;n++)
          if (n!=MyID) {
              ...
              MPI_Send(&k, sizeof(int), MPI_CHAR, n, MULTI_LEN+MyID,
                       MPI_COMM_WORLD);
          }
      } else {
        MPI_Recv(&temp1[i], sizeof(int), MPI_CHAR, i,MULTI_LEN+i,
    ...
    for (i=0;i<SumID;i++) {
      ...
      if (i==MyID) {
        for (n=0;n<SumID;n++)
          if (n!=MyID) {
            MPI_Send(&arr[index[2*n]], sizeof(int)*(index[2*n+1]-index[2*n]),
                     MPI_CHAR, n, MULTI_TYPE+MyID, MPI_COMM_WORLD);
          }

      } else {
        l = temp1[i];
        MPI_Recv(&arr1[k], l*sizeof(int), MPI_CHAR,
...
/*串行多路归并算法*/
void multimerge(int *data1,int * ind ,int *data,int *iter,int
                      SumID)
{
         int i,j,n;
         j = 0;
